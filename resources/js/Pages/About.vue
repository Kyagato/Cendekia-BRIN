  ...
  <PublicLayout>
        ...
        <nav class="flex justify-center text-sm text-white/70 mb-4" aria-label="Breadcrumb">
          <ol class="inline-flex items-center space-x-1 md:space-x-3">
            <li class="inline-flex items-center">
              <Link href="/" class="inline-flex items-center hover:text-white transition">
                Beranda
              </Link>
            </li>
            <li>
    ...
    <section class="py-16 container mx-auto px-4">
      <div class="flex flex-col lg:flex-row items-center gap-12">
        <div class="lg:w-1/2">
          <h2 class="text-3xl font-bold text-slate-800 dark:text-white mb-6">Apa itu <span class="text-red-500">MojoPedia?</span></h2>
        ...
        <div class="lg:w-1/2">
          <div class="relative w-full h-80 rounded-2xl bg-gradient-to-tr from-red-600 to-rose-400 p-1 overflow-hidden shadow-2xl">
            <div class="relative z-30 w-full h-full bg-white dark:bg-slate-900 rounded-xl flex items-center justify-center p-8">
              <div class="text-center">
                <svg class="w-24 h-24 text-red-500 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <section class="py-16 bg-slate-950/50">
      <div class="container mx-auto px-4">
        <h2 class="text-3xl font-bold text-center text-slate-800 dark:text-white mb-12">Cara Menggunakan</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div v-for="(step, idx) in steps" :key="idx" class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 relative">
            <div class="w-12 h-12 bg-red-600 text-white rounded-full flex items-center justify-center text-xl font-bold absolute -top-6 left-6 shadow-lg">{{ idx + 1 }}</div>
            <h3 class="text-xl font-bold text-slate-800 dark:text-white mt-4 mb-2">{{ step.title }}</h3>
    ...
    <!-- Fitur Utama -->
    <section class="py-16 container mx-auto px-4">
      <h2 class="text-3xl font-bold text-center text-slate-800 dark:text-white mb-12">Fitur Utama</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="(feat, idx) in features" :key="idx" class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-start gap-4">
          <div class="w-12 h-12 bg-red-500/10 rounded-lg flex items-center justify-center text-red-500 shrink-0">
