<template>
  <PublicLayout>
    <div class="py-8 container mx-auto px-4 max-w-4xl space-y-6">
      ...
      <!-- Thread Card -->
      <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-[#e2e8f0] dark:border-slate-800 overflow-hidden" :class="thread.is_pinned ? 'border-[#2563eb]' : ''">
        <div class="p-6 sm:p-8">
          <!-- Meta Badges -->
          <div class="flex flex-wrap items-center gap-2 text-sm text-[#94a3b8] dark:text-slate-400 mb-4">
            <span v-if="thread.is_pinned" class="bg-[#eff6ff] text-[#2563eb] dark:bg-blue-950 dark:text-blue-300 text-xs px-2.5 py-0.5 rounded-full font-semibold border border-blue-200/60">
              📌 Pinned
            </span>
            <span v-if="thread.is_locked" class="bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300 text-xs px-2.5 py-0.5 rounded-full font-semibold">
              🔒 Dikunci
            </span>
            <span v-if="thread.category" class="bg-[#f1f5f9] dark:bg-slate-800 text-[#475569] dark:text-slate-300 px-2.5 py-0.5 rounded-full text-xs font-medium border border-[#e2e8f0] dark:border-slate-700">
          ...
          <div v-if="thread.knowledge" class="mb-6 p-4 bg-[#eff6ff]/70 dark:bg-blue-950/30 border border-blue-200/80 dark:border-blue-900/60 rounded-lg flex items-center justify-between">
            <div class="flex items-center gap-3">
              <div class="p-2 bg-blue-100 dark:bg-blue-900/50 text-[#2563eb] dark:text-blue-400 rounded-lg shrink-0">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
              </div>
              <div>
                <span class="text-[11px] font-bold text-[#2563eb] dark:text-blue-400 uppercase tracking-wider block">Membahas Pengetahuan</span>
                <a :href="`/knowledge/${thread.knowledge.id}`" class="text-sm font-semibold text-[#0f172a] dark:text-slate-200 hover:text-[#2563eb] transition">
                  {{ thread.knowledge.judul }}
                </a>
              </div>
            </div>
            <a :href="`/knowledge/${thread.knowledge.id}`" class="shrink-0 px-3 py-1.5 bg-[#2563eb] hover:bg-[#1d4ed8] text-white text-xs font-semibold rounded-lg transition shadow-sm">
              Lihat Materi
            </a>
          </div>
      ...
      <!-- Reply Count -->
      <h2 class="text-lg font-bold text-[#0f172a] dark:text-slate-100">
      ...
      <div v-else-if="!user" class="text-center py-8 bg-white dark:bg-slate-900 rounded-lg border border-[#e2e8f0] dark:border-slate-800 shadow-sm">
        <p class="text-sm text-[#475569] dark:text-slate-400 mb-3">Silakan masuk untuk memberikan balasan.</p>
