...
@section('breadcrumbs')
    ...
    <li class="text-slate-800 dark:text-slate-200 font-semibold">Pengguna</li>
@endsection

@section('content')
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100">Manajemen Pengguna Sistem</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola data pengguna, peran, dan hak akses sistem.</p>
    </div>
    <div class="flex items-center gap-3 w-full sm:w-auto">
        ...
        <form action="{{ route('admin.users.index') }}" method="GET" class="relative w-full sm:w-64">
            ...
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari pengguna..." class="w-full pl-9 pr-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-red-600 focus:border-red-600 transition">
        </form>
        ...
        <table class="w-full text-left border-collapse min-w-max">
            <thead>
                <tr class="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
                    <th class="py-3 px-6 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nama Lengkap & Email</th>
                    <th class="py-3 px-6 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Instansi</th>
                    <th class="py-3 px-6 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Peran / Role</th>
                    <th class="py-3 px-6 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-right">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700 transition">
                    <td class="py-4 px-6">
                        <div class="flex items-center gap-3">
                            ...
                            @else
                                <div class="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-700 dark:text-slate-200 font-bold shrink-0">
                    ...
                    <td class="py-4 px-6 text-sm text-slate-600 dark:text-slate-300">{{ $user->instansi ?? '-' }}</td>
                    <td class="py-4 px-6">
                        @php
                            $roleClass = match($user->role) {
                                ...
                                'Admin' => 'bg-orange-100 dark:bg-orange-950 text-orange-800 dark:text-orange-300 border-orange-200 dark:border-orange-800',
                    ...
                    <td class="py-4 px-6 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengguna ini?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="inline-flex items-center px-3 py-1.5 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 hover:border-red-300 rounded text-xs font-medium transition" {{ auth()->id() === $user->id ? 'disabled' : '' }}>
                                    Hapus
                                </button>
                            </form>
                            @else
                ...
                @empty
                <tr>
                    <td colspan="4" class="py-8 px-6 text-center text-slate-500 dark:text-slate-400">Belum ada data pengguna.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
