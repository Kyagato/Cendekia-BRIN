                ...
                <div class="lg:col-span-7 space-y-6">
                    ...
                    <div>
                        ...
                        <template x-if="formatType === 'Gambar'">
                            <div x-data="{ imgFileName: '' }">
                                ...
                                @if($knowledge->file_path && $knowledge->tipe === 'Gambar')
                                <div class="mb-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700">
                                    <p class="text-xs text-slate-500 dark:text-slate-400 mb-2">Gambar saat ini:</p>
                                    <img src="{{ asset('storage/' . $knowledge->file_path) }}" alt="Preview" class="max-h-32 rounded-lg object-contain">
                                </div>
                                @endif
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 dark:border-slate-600 border-dashed rounded-lg bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition cursor-pointer" onclick="document.getElementById('gambar_upload_edit').click()">
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-slate-600 dark:text-slate-400 justify-center mt-2">
                                            <label for="gambar_upload_edit" class="relative cursor-pointer bg-transparent rounded-md font-medium text-primary-600 dark:text-primary-400 hover:text-primary-500 dark:hover:text-primary-300">
                                                <span x-show="!imgFileName">{{ $knowledge->file_path && $knowledge->tipe === 'Gambar' ? 'Ganti gambar' : 'Pilih file gambar' }}</span>
                                                ...
                                                <input id="gambar_upload_edit" name="file_upload" type="file" accept="image/*" class="sr-only" @change="imgFileName = $event.target.files[0] ? $event.target.files[0].name : ''">
                                            </label>
                                        </div>
                                        <p x-show="!imgFileName" class="text-xs text-slate-500 dark:text-slate-400 mt-1">PNG, JPG, GIF, WEBP — Maks 5MB</p>
                                        <p x-cloak x-show="imgFileName" class="text-xs text-green-600 dark:text-green-400 font-medium mt-1">Gambar baru siap diupload</p>
                                    </div>
                                </div>
                        ...
                        <template x-if="formatType === 'Audio'">
                            <div x-data="{ audioFileName: '' }">
                                ...
                                @php
                                    $hasAudioFile = $knowledge->tipe === 'Audio' && ($knowledge->url_teks || $knowledge->file_path);
                                    $audioSrc = $knowledge->url_teks ?? $knowledge->file_path;
                                @endphp
                                @if($hasAudioFile)
                                <div class="mb-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700">
                                    <p class="text-xs text-slate-500 dark:text-slate-400 mb-2">Audio saat ini:</p>
                                    <audio controls class="w-full">
                                        <source src="{{ str_starts_with($audioSrc, 'http') ? $audioSrc : asset('storage/' . $audioSrc) }}">
                                        Browser Anda tidak mendukung pemutar audio.
                                    </audio>
                                </div>
                                @endif
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 dark:border-slate-600 border-dashed rounded-lg bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition cursor-pointer" onclick="document.getElementById('audio_upload_edit').click()">
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-slate-600 dark:text-slate-400 justify-center mt-2">
                                            <label for="audio_upload_edit" class="relative cursor-pointer bg-transparent rounded-md font-medium text-primary-600 dark:text-primary-400 hover:text-primary-500 dark:hover:text-primary-300">
                                                <span x-show="!audioFileName">{{ $hasAudioFile ? 'Ganti audio' : 'Pilih file audio' }}</span>
                                                ...
                                                <input id="audio_upload_edit" name="audio_file" type="file" accept="audio/*" class="sr-only" @change="audioFileName = $event.target.files[0] ? $event.target.files[0].name : ''">
                                            </label>
                                        </div>
                                        <p x-show="!audioFileName" class="text-xs text-slate-500 dark:text-slate-400 mt-1">MP3, WAV, OGG, M4A, AAC, FLAC — Maks 10MB</p>
                                        <p x-cloak x-show="audioFileName" class="text-xs text-green-600 dark:text-green-400 font-medium mt-1">Audio baru siap diupload</p>
                                    </div>
                                </div>
                        ...
                        <template x-if="formatType !== 'Gambar' && formatType !== 'Audio'">
                            <div>
                                ...
                                <input type="url" id="url_teks" name="url_teks" value="{{ old('url_teks', $knowledge->url_teks) }}" :placeholder="'Masukkan url ' + formatType" class="w-full px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-600 focus:ring-primary-600 focus:border-primary-600 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" id="penulis" name="penulis" value="{{ old('penulis', $knowledge->penulis) }}" placeholder="Masukkan nama penulis" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 focus:ring-primary-600 focus:border-primary-600 bg-white dark:bg-slate-900 text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500" required>
                        ...
                        <div>
                            ...
                            <input type="text" id="kolaborator" name="kolaborator" value="{{ old('kolaborator', $knowledge->kolaborator) }}" placeholder="Masukkan nama kolaborator" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 focus:ring-primary-600 focus:border-primary-600 bg-white dark:bg-slate-900 text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500">
                    ...
                    <div>
                        ...
                        <x-rich-editor name="deskripsi" id="deskripsi" :value="old('deskripsi', $knowledge->deskripsi)" placeholder="Tuliskan ringkasan pengetahuan di sini..." :rows="4" />
                    ...
                    <div>
                        ...
                        <x-rich-editor name="detail" id="detail" :value="old('detail', $knowledge->detail)" placeholder="Tuliskan detail pengetahuan di sini..." :rows="6" />
                ...
                <div class="lg:col-span-5 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_terbit" value="{{ old('tanggal_terbit', $knowledge->tanggal_terbit ? \Carbon\Carbon::parse($knowledge->tanggal_terbit)->format('Y-m-d') : '') }}" class="w-full px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-600 focus:ring-primary-600 focus:border-primary-600 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300">
                        </div>
                    </div>

                    {{-- Baris 2: Tag & Status Akses --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {{-- Tag --}}
                        @php
                        ...
                        <div x-data="{
                            ...
                            <div class="flex flex-wrap items-center gap-2 min-h-[48px] px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus-within:ring-2 focus-within:ring-primary-600 focus-within:border-primary-600 transition">
                                ...
                                <input type="text" x-model="tagInput"
                                       ...
                                       class="flex-1 min-w-[120px] border-0 bg-transparent p-0 text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-0 focus:outline-none">
                            </div>
                        </div>
                    ...
                    <div class="flex items-center gap-3 py-3 border-b border-slate-100 dark:border-slate-700">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="unggulan" value="1" class="w-5 h-5 text-primary-600 bg-slate-100 dark:bg-slate-900 border-slate-300 dark:border-slate-600 rounded focus:ring-primary-600 focus:ring-2" {{ old('unggulan', $knowledge->unggulan) ? 'checked' : '' }}>
                            <span class="ml-2 text-sm font-bold text-slate-800 dark:text-slate-200">Jadikan Pengetahuan Unggulan</span>
                        </label>
                    </div>

                    {{-- Thumbnail Upload (tersembunyi hanya saat format Gambar) --}}
                    <div x-show="formatType !== 'Gambar'" x-data="{ fileName: '' }">
                        ...
                        @php
                            $ext = $knowledge->file_path ? strtolower(pathinfo($knowledge->file_path, PATHINFO_EXTENSION)) : '';
                            $isThumbnailImg = in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp']);
                        @endphp
                        @if($knowledge->file_path && $isThumbnailImg)
                            <div class="mb-4">
                                <p class="text-sm text-slate-500 dark:text-slate-400 mb-2">Thumbnail saat ini:</p>
                                <div class="p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg flex items-center gap-3">
                                    <img src="{{ asset('storage/' . $knowledge->file_path) }}" alt="Thumbnail" class="w-16 h-16 object-cover rounded-lg">
                                    <span class="text-sm font-medium text-slate-700 dark:text-slate-300 truncate">{{ basename($knowledge->file_path) }}</span>
                                </div>
                            </div>
                        @elseif($knowledge->file_path && !$isThumbnailImg && $knowledge->tipe !== 'Audio')
                            <div class="mb-4">
