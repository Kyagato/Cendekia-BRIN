...
<div class="max-w-6xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">{{ $knowledge->judul }}</h1>
    ...
    <div class="bg-white shadow-md rounded-xl overflow-hidden">
        ...
        {{-- Meta Info Bar --}}
        <div class="px-8 py-5 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center gap-4">
            {{-- Status Badge --}}
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold
                {{ $knowledge->status == 'Disetujui' ? 'bg-green-100 text-green-700 border border-green-200' : 
                  ($knowledge->status == 'Ditolak' ? 'bg-red-100 text-red-700 border border-red-200' : 
                  ($knowledge->status == 'Diajukan' ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-yellow-100 text-yellow-700 border border-yellow-200')) }}">
                {{ $knowledge->status }}
            </span>
            ...
            @if($knowledge->tags && $knowledge->tags->count())
                ...
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-600">#{{ $tag->nama_label }}</span>
                @endforeach
            @endif
        </div>
        ...
        <div class="p-8">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-8 space-y-6">
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-slate-800 mb-2 uppercase tracking-wider">Ringkasan</h2>
                        <div class="text-slate-700 leading-relaxed text-sm bg-slate-50 rounded-lg p-4 border border-slate-200">
                    ...
                    @if($knowledge->detail)
                    <div>
                        <h2 class="text-sm font-bold text-slate-800 mb-2 uppercase tracking-wider">Detail</h2>
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-slate-800 mb-2 uppercase tracking-wider">URL {{ $knowledge->tipe }}</h2>
                        <a href="{{ $knowledge->url_teks }}" target="_blank" class="inline-flex items-center gap-2 text-sm text-red-600 hover:text-red-700 hover:underline break-all">
                ...
                <div class="lg:col-span-4 space-y-5">
                    <div class="bg-slate-50 rounded-xl p-5 border border-slate-200 space-y-4">
                        <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider">Informasi</h3>
                        
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between">
                                <span class="text-slate-500">Penulis</span>
                                <span class="text-slate-800 font-medium">{{ $knowledge->penulis ?? $knowledge->user->name ?? '-' }}</span>
                            </div>
                            @if($knowledge->kolaborator)
                            <div class="flex justify-between">
                                <span class="text-slate-500">Kolaborator</span>
                                <span class="text-slate-800 font-medium">{{ $knowledge->kolaborator }}</span>
                            </div>
                            @endif
                            <div class="flex justify-between">
                                <span class="text-slate-500">Tanggal Dibuat</span>
                                <span class="text-slate-800 font-medium">{{ $knowledge->created_at?->format('d M Y') ?? '-' }}</span>
                            </div>
                            @if($knowledge->tanggal_terbit)
                            <div class="flex justify-between">
                                <span class="text-slate-500">Tanggal Terbit</span>
                                <span class="text-slate-800 font-medium">{{ $knowledge->tanggal_terbit }}</span>
                            </div>
                            @endif
                            <div class="flex justify-between">
                                <span class="text-slate-500">Status Akses</span>
                                <span class="text-slate-800 font-medium capitalize">{{ $knowledge->status_akses ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">Dilihat</span>
                                <span class="text-slate-800 font-medium">{{ $knowledge->views_count ?? 0 }}×</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-red-50 rounded-xl p-5 border border-red-200">
                        ...
                        <form action="{{ route('knowledge.destroy', $knowledge->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengetahuan ini secara permanen?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full px-4 py-2 border-2 border-red-300 text-red-600 hover:bg-red-100 rounded-lg text-sm font-medium transition">
                                Hapus Pengetahuan
                            </button>
                        </form>
                    </div>
