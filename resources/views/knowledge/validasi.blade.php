...
@section('breadcrumbs')
    ...
    <li class="text-slate-800 dark:text-slate-200 font-semibold">Tinjau &amp; Validasi</li>
@endsection
...
<div x-data="{ showTolakModal: false }" class="max-w-6xl mx-auto">
    ...
    @if(session('success'))
    <div class="mb-4 p-4 bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-300 rounded-lg flex items-center gap-2 text-sm shadow-sm">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100">Validasi &amp; Ubah Pengetahuan</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Tinjau konten sebelum memberikan keputusan persetujuan</p>
        </div>
        <a href="{{ route('validasi.index') }}" class="px-5 py-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition flex items-center gap-2 shadow-sm">
    ...
    <div class="bg-white dark:bg-slate-800 shadow-md rounded-xl p-8 border border-slate-200 dark:border-slate-700">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-7 space-y-6">
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-2">Judul</label>
                    <div class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-medium break-words [overflow-wrap:anywhere]">{{ $knowledge->judul }}</div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-x-6 gap-y-3">
                        ...
                        <label class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-medium text-slate-700 dark:text-slate-300">{{ $fmt === 'Teks' ? 'Text' : $fmt }}</span>
                        </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                        <a href="{{ $knowledge->url_teks }}" target="_blank" class="text-red-600 dark:text-red-400 hover:underline text-sm break-all">{{ $knowledge->url_teks }}</a>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-sm text-slate-800 dark:text-slate-100 font-medium">{{ $knowledge->penulis ?? '-' }}</div>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-2">Kolaborator</label>
                        <div class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-sm text-slate-800 dark:text-slate-100 font-medium">{{ $knowledge->kolaborator ?? '-' }}</div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 text-sm leading-relaxed min-h-[80px] break-words [overflow-wrap:anywhere]">{!! nl2br(e($knowledge->deskripsi)) !!}</div>
                </div>
                @endif

                @if($knowledge->detail)
                <div>
                    <label class="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-2">Detail</label>
                    <div class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 text-sm leading-relaxed min-h-[120px] break-words [overflow-wrap:anywhere]">{!! nl2br(e($knowledge->detail)) !!}</div>
                </div>
                @endif
                ...
                <div>
                    ...
                    <div class="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg flex items-center gap-3">
                        <img src="{{ asset('storage/' . $knowledge->file_path) }}" alt="Thumbnail" class="w-16 h-16 object-cover rounded border border-slate-200 dark:border-slate-700">
                        <p class="text-sm text-slate-600 dark:text-slate-400">File terpasang</p>
                    </div>
                </div>
                @endif
            ...
            <div class="lg:col-span-5 space-y-6">
                ...
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-900">
                    <div class="px-5 py-3 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200">Persetujuan</h3>
                    </div>
                    <div class="px-5 py-4 flex flex-wrap gap-2">
                        <form action="{{ route('validasi.approve', $knowledge->id) }}" method="POST" class="flex-1">
                            ...
                            <button type="submit"
                                    ...
                                    class="w-full px-4 py-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-sm font-semibold rounded-lg transition flex items-center justify-center gap-2 shadow-sm">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                                Setujui &amp; Terbit
                            </button>
                        </form>
                        <button type="button" @click="showTolakModal = true"
                                class="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white text-sm font-semibold rounded-lg transition flex items-center justify-center gap-2 shadow-sm">
                ...
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-900">
                    <div class="px-5 py-3 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200">Status</h3>
                    </div>
                ...
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-900">
                    ...
                    <div class="px-5 py-4 flex items-center justify-between">
                        <span class="text-sm text-slate-700 dark:text-slate-300">{{ $knowledge->category->nama_kategori ?? '-' }}</span>
                        <svg class="w-4 h-4 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                    </div>
                </div>
                ...
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-900">
                    <div class="px-5 py-3 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200">Label</h3>
                    </div>
                    <div class="px-5 py-4 flex flex-wrap gap-2">
                ...
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-900">
                    ...
                    <div class="px-5 py-4">
                        <div class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-sm text-slate-700 dark:text-slate-300">
