...
<section class="pt-32 pb-12 bg-slate-50 dark:bg-slate-900 min-h-screen">
    <div class="container mx-auto px-4 max-w-4xl">
        ...
        <div x-data="{ show: true }" 
             ...
             class="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 text-green-700 dark:text-green-300 px-4 py-3 rounded-lg mb-4 flex items-center justify-between shadow-sm">
            ...
            <button type="button" @click="show = false" class="text-green-500 hover:text-green-700 dark:hover:text-green-200 transition">
        ...
        <div x-data="{ show: true }" 
             ...
             x-transition:leave-start="opacity-100 transform scale-100"
             x-transition:leave-end="opacity-0 transform scale-95"
             class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 text-red-700 dark:text-red-300 px-4 py-3 rounded-lg mb-4 flex items-center justify-between shadow-sm">
            <div class="flex items-center gap-2">
            ...
            <button type="button" @click="show = false" class="text-red-500 hover:text-red-700 dark:hover:text-red-200 transition">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
        </div>
        @endif
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden mb-8 {{ $thread->is_pinned ? 'ring-2 ring-yellow-400 dark:ring-yellow-500' : '' }}">
            <div class="p-6 sm:p-8">
                ...
                @if($thread->knowledge)
                    <div class="mb-6 p-4 bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900/50 rounded-xl flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <a href="{{ route('knowledge.show', $thread->knowledge->id) }}" class="text-sm font-semibold text-slate-800 dark:text-slate-200 hover:text-red-600 transition">
                                    {{ $thread->knowledge->judul }}
                                </a>
                            </div>
                        </div>
                        <a href="{{ route('knowledge.show', $thread->knowledge->id) }}" class="text-xs bg-red-100 dark:bg-red-900/50 hover:bg-red-200 text-red-700 dark:text-red-300 font-semibold px-3 py-1.5 rounded-lg transition shrink-0">
                            Lihat Materi
                        </a>
                    </div>
                @endif
